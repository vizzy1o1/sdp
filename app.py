...
#             jaccard_sim = 1 - jaccard_distance(set(word), set(reference_word))
#             if jaccard_sim >= 0.8:
    ...
    for word in filtered_words:
        ...
        for reference_word in reference_data:
            ...
            if similarity >= 0.8: 
                ...
                break
        if not modified_word is None:
            modified_words.append(modified_word) #we're just removing words that dont match to make it easier (scope for imprvement)
    return ' '.join(modified_words)
